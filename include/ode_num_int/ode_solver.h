...
class ScaledErrorCalculatorUtil
    {
    protected:
        template< class VD >
        static void loadScaling(
                VectorTemplate<VD>& dst, const std::string& fileName, const std::vector<unsigned int>& ids )
            {
            Vector<double> buf;
            loadScalingPriv( buf, fileName, ids );
            dst = buf;
            }

        static void loadScaling(
                Vector<double>& dst, const std::string& fileName, const std::vector<unsigned int>& ids )
            {
            loadScalingPriv( dst, fileName, ids );
            }

    private:
        static void loadScalingPriv(
                Vector<double>& dst, const std::string& fileName, const std::vector<unsigned int>& ids );
    };

template< class VD >
class ScaledErrorCalculator :
    public ErrorNormCalculator<VD>,
    public ErrorNormCalculatorHolder<VD>,
    public ScaledErrorCalculatorUtil,
    public FactoryMixin< ScaledErrorCalculator<VD>, ErrorNormCalculator<VD> >
    {
    public:
        typedef VectorTemplate< VD > V;
        typedef typename VD::value_type real_type;
        ...
        ScaledErrorCalculator() :
            m_scalingDataValid( false )
            {
            auto invalidateScaling = [this] {
                m_scalingDataValid = false;
                };
            this->onErrorNormCalculatorChanged( invalidateScaling );
            this->onOdeRhsChanged( invalidateScaling );
            this->setErrorNormCalculator( Factory< ErrorNormCalculator<VD> >::newInstance( "inf" ) );
            }

        std::string scalingDataFileName() const {
            return m_scalingDataFileName;
            }

        void setScalingDataFileName( const std::string& fileName )
            {
            if( m_scalingDataFileName != fileName ) {
                m_scalingDataValid = false;
                m_scalingDataFileName = fileName;
                }
            }

        real_type errorNorm( const V& error ) const
            {
            auto errorNorm = this->errorNormCalculator()->errorNorm( scale( error ) );
            this->errorNormCalculatorObservers( error, errorNorm );
            return errorNorm;
            }
        ...
        Parameters parameters() const
            ...
            parameters["ref_enorm"] = this->errorNormCalculator();
            parameters["scaling_data_file"] = m_scalingDataFileName;
            return parameters;
            }
        ...
        void setParameters( const Parameters& parameters )
            ...
            OptionalParameters::maybeSetParameter(
                        parameters, "ref_enorm",
                        std::bind( &ErrorNormCalculatorHolder<VD>::setErrorNormCalculator, this, std::placeholders::_1 ) );
            OptionalParameters::maybeSetParameter(
                        parameters, "scaling_data_file",
                        std::bind( &ScaledErrorCalculator<VD>::setScalingDataFileName, this, std::placeholders::_1 ) );
            }

        virtual Parameters helpOnParameters() const
            {
            Parameters parameters;
            parameters["ref_enorm"] = OptionalParameters::appendNestedHelp(
                        this->errorNormCalculator(),
                        "Reference error norm calculator" );
            parameters["scaling_data_file"] = "Name of file containing scaling data. Each line is\n"
                                              "value-id TAB characteristic-error-value";
            return parameters;
            }

    private:
        std::string m_scalingDataFileName;
        mutable V m_buf;
        mutable V m_scalingData;
        mutable bool m_scalingDataValid;

        V& scalingData() const
            {
            if( !m_scalingDataValid ) {
                m_scalingDataValid = true;
                this->loadScaling( m_scalingData, m_scalingDataFileName, this->odeRhs()->varIds() );
                }
            return m_scalingData;
            }

        V& scale( const V& error ) const
            {
            m_buf = error;
            auto n = error.size();
            auto buf = m_buf.begin();
            auto s = scalingData().begin();
            for( decltype(n) i=0; i<n; ++i, ++buf, ++s )
                *buf *= *s;
            return m_buf;
            }
    };
